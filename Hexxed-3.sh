...
# Get wallpapers
wget -O ~/.config/wallpaper/wallpaper.jpg https://www.hexxedbitheadz.com/wp-content/uploads/go-x/u/77625540-30c4-410a-8d61-39a87bd37707/image.jpg && wget -O ~/.config/wallpaper/lockscreen.png https://www.hexxedbitheadz.com/wp-content/uploads/go-x/u/ab79c8dd-2b3b-47a0-8ed6-9176861e002b/image.png
...
sed -i 's/foreground = #C5C8C6/foreground = #000000/g' ~/.config/polybar/config.ini
sed -i 's/primary = #F0C674/primary = #ef0b16/g' ~/.config/polybar/config.ini
sed -i 's/secondary = #8ABEB7/secondary = #000000/g' ~/.config/polybar/config.ini
sed -i '/disabled =/a cancelButton = #5d5754' ~/.config/polybar/config.ini
...
sed -i 's/label-connected = %{F#F0C674}%ifname%%{F-} %essid% %local_ip%/label-connected = %{F#ef0b16}%ifname%%{F-} %essid% %local_ip%/g' ~/.config/polybar/config.ini

sed -i 's/%{F#F0C674}%ifname%%{F-} %local_ip%/%{F#ef0b16}%ifname%%{F-} %local_ip%/g' ~/.config/polybar/config.ini
...
sed -i '/\[urgency_low\]/,/timeout = 10/ { s/background = "#222222"/background = "#5d5754"/; s/foreground = "#888888"/foreground = "#ffffff"/; s/# IMPORTANT: colors have to be defined in quotation marks./# IMPORTANT: colors have to be defined in quotation marks./; s/timeout = 10/timeout = 10/; s/#default_icon = \/path\/to\/icon/#default_icon = \/path\/to\/icon/; s/\[urgency_low\]/\[urgency_low\]\n    frame_color = "#000000"/; }
/\[urgency_normal\]/,/timeout = 10/ { s/background = "#285577"/background = "#fefdfd"/; s/foreground = "#ffffff"/foreground = "#b86055"/; s/timeout = 10/timeout = 10/; s/#default_icon = \/path\/to\/icon/#default_icon = \/path\/to\/icon/; s/\[urgency_normal\]/\[urgency_normal\]\n    frame_color = "#000000"/; }
/\[urgency_critical\]/,/timeout = 0/ { s/background = "#900000"/background = "#ef0b16"/; s/foreground = "#ffffff"/foreground = "#000000"/; s/frame_color = "#ff0000"/frame_color = "#b86055"/; s/timeout = 0/timeout = 0/; s/#default_icon = \/path\/to\/icon/#default_icon = \/path\/to\/icon/; }' ~/.config/dunst/dunstrc
