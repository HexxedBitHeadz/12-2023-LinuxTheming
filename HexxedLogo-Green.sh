...
# Get wallpapers
wget -O ~/.config/wallpaper/wallpaper.jpg https://www.hexxedbitheadz.com/wp-content/uploads/go-x/u/8c19f7f2-30c5-4b8a-91e1-2491b94fa55c/image.jpg && wget -O ~/.config/wallpaper/lockscreen.png https://www.hexxedbitheadz.com/wp-content/uploads/go-x/u/6b2368e9-520c-4f95-b3f8-c46d0ee43731/image.png
...
sed -i 's/foreground = #C5C8C6/foreground = #8eca8e/g' ~/.config/polybar/config.ini
sed -i 's/primary = #F0C674/primary = #1b311a/g' ~/.config/polybar/config.ini
sed -i 's/secondary = #8ABEB7/secondary = #000000/g' ~/.config/polybar/config.ini
sed -i '/disabled =/a cancelButton = #1b311a' ~/.config/polybar/config.ini
...
sed -i 's/border-color = #00000000/border-color = #1b311a/' ~/.config/polybar/config.ini
...
sed -i 's/label-connected = %{F#F0C674}%ifname%%{F-} %essid% %local_ip%/label-connected = %{F#1b311a}%ifname%%{F-} %essid% %local_ip%/g' ~/.config/polybar/config.ini

sed -i 's/%{F#F0C674}%ifname%%{F-} %local_ip%/%{F#1b311a}%ifname%%{F-} %local_ip%/g' ~/.config/polybar/config.ini
...
sed -i '/\[urgency_low\]/,/timeout = 10/ { s/background = "#222222"/background = "#000000"/; s/foreground = "#888888"/foreground = "#8eca8e"/; s/# IMPORTANT: colors have to be defined in quotation marks./# IMPORTANT: colors have to be defined in quotation marks./; s/timeout = 10/timeout = 10/; s/#default_icon = \/path\/to\/icon/#default_icon = \/path\/to\/icon/; s/\[urgency_low\]/\[urgency_low\]\n    frame_color = "#8eca8e"/; }
