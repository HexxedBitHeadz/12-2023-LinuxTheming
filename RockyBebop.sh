...
# Get wallpapers
wget -O ~/.config/wallpaper/wallpaper.jpg https://www.hexxedbitheadz.com/wp-content/uploads/go-x/u/b2842b79-422a-4c54-81a6-00e97cd6ddd8/image.jpg && wget -O ~/.config/wallpaper/lockscreen.png https://www.hexxedbitheadz.com/wp-content/uploads/go-x/u/2564695a-4743-4054-a29e-d9ae61b88a25/image.png
...
# Customizing polybar:
sed -i 's/background = #282A2E/background = #000000/g' ~/.config/polybar/config.ini
sed -i 's/background-alt = #373B41/background-alt = #000000/g' ~/.config/polybar/config.ini
sed -i 's/foreground = #C5C8C6/foreground = #0c75a0/g' ~/.config/polybar/config.ini
sed -i 's/primary = #F0C674/primary = #7115b5/g' ~/.config/polybar/config.ini
sed -i 's/secondary = #8ABEB7/secondary = #87d63c/g' ~/.config/polybar/config.ini
sed -i '/disabled =/a cancelButton = #7115b5' ~/.config/polybar/config.ini
...
sed -i 's/label-connected = %{F#F0C674}%ifname%%{F-} %essid% %local_ip%/label-connected = %{F#7115b5}%ifname%%{F-} %essid% %local_ip%/g' ~/.config/polybar/config.ini

sed -i 's/%{F#F0C674}%ifname%%{F-} %local_ip%/%{F#7115b5}%ifname%%{F-} %local_ip%/g' ~/.config/polybar/config.ini
...
sed -i '/\[urgency_low\]/,/timeout = 10/ { s/background = "#222222"/background = "#0c75a0"/; s/foreground = "#888888"/foreground = "#7115b5"/; s/# IMPORTANT: colors have to be defined in quotation marks./# IMPORTANT: colors have to be defined in quotation marks./; s/timeout = 10/timeout = 10/; s/#default_icon = \/path\/to\/icon/#default_icon = \/path\/to\/icon/; s/\[urgency_low\]/\[urgency_low\]\n    frame_color = "#000000"/; }
...
/\[urgency_critical\]/,/timeout = 0/ { s/background = "#900000"/background = "#fb00c8"/; s/foreground = "#ffffff"/foreground = "#000000"/; s/frame_color = "#ff0000"/frame_color = "#7115b5"/; s/timeout = 0/timeout = 0/; s/#default_icon = \/path\/to\/icon/#default_icon = \/path\/to\/icon/; }' ~/.config/dunst/dunstrc
